...
void MainWindow::on_s0_pb_Login_clicked()
{
    ...
    if(users->check_credentials(username, password))
    {
        ...
        std::string fName = username + ".txt";
        hiveDB->populate(fName);
...
void MainWindow::on_s3_pb_beeChimes_clicked()
{
    ...
    long scale = pow(2, ui->s3_hs_scale->value());
    float pSimPlayed = ui->s3_hs_pSimPlayed->value() / 100;
    // NEED TO CHANGE
...
{
    Alert al;
    std::fstream file("/Users/ricdonati/desktop/build-qtrepo_grp00_may-Desktop_Qt_5_12_0_clang_64bit-Debug/CustomerGUI/example.txt");
...
void MainWindow::on_s3_pb_weather_clicked()
{
    ww = new WeatherWidget;
    ui->stackedWidget->insertWidget(5, ww);
    ui->stackedWidget->setCurrentIndex(5);
    connect(ww, SIGNAL(backClicked()), this, SLOT(recieve_backClicked_weather()));
}

void MainWindow::recieve_backClicked_weather()
{
